...
export const PaymentCheckout = ({ appointmentId, amount, onSuccessClose }: Props) => {
  ...
  return (
    <div style={{ maxWidth: '400px', padding: '20px', border: '1px solid #ddd', borderRadius: '8px', backgroundColor: '#fff' }}>
      <h2 style={{ marginTop: '0' }}>Finalizacja płatności</h2>
      <p>Kwota do zapłaty: <strong style={{ fontSize: '18px', color: '#007bff' }}>{amount.toFixed(2)} PLN</strong></p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '20px' }}>
        ...
        <label style={{
          ...
          padding: '15px', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: selectedMethod === 'PAYU' ? '#f0f7ff' : '#fff'
        }}>
          <input 
            type="radio" 
            name="payMethod" 
        ...
        <label style={{
          border: selectedMethod === 'OFFLINE' ? '2px solid #007bff' : '1px solid #ccc',
          padding: '15px', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: selectedMethod === 'OFFLINE' ? '#f0f7ff' : '#fff'
        }}>
          <input 
            type="radio" 
            name="payMethod" 
      ...
      <button 
        ...
        style={{ 
          ...
          color: 'white', border: 'none', borderRadius: '6px', fontSize: '16px', fontWeight: 'bold', cursor: selectedMethod ? 'pointer' : 'not-allowed'
        }}
      >
        {isLoading ? 'Przetwarzanie...' : (selectedMethod === 'PAYU' ? 'Przejdź do PayU' : 'Potwierdzam rezerwację')}
      </button>
    </div>
  );
};