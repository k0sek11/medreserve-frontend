    ...
    return (
      <div className="min-h-screen bg-slate-50 py-12 px-4 flex justify-center">
        <div className="max-w-xl w-full bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
          <Alert severity="info" className="rounded-xl">Ładowanie podsumowania wizyty...</Alert>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 py-12 px-4 flex justify-center">
        <div className="max-w-md w-full bg-white p-8 rounded-3xl border border-slate-100 shadow-sm text-center">
          ...
          <h2 className="text-xl font-bold text-slate-800">Nie udało się pobrać podsumowania</h2>
        </div>
      </div>
    );
  }
        ...
        <Stack spacing={4}>
          ...
          <div className="text-center mt-2">
            <div className="inline-flex items-center justify-center w-20 h-20 bg-emerald-50 rounded-full mb-4">
              <span className="text-4xl">{appointment.status === "Confirmed" ? "✅" : "⏳"}</span>
            </div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight mb-2">
              {title}
            </h1>
            <p className="text-slate-500 font-medium">
              Poniżej znajdziesz unikalny numer rezerwacji oraz najważniejsze informacje o Twojej wizycie.
            </p>
          </div>
          ...
          <div className="flex justify-center">
            <Chip 
              ...
              sx={{ fontWeight: 800, fontSize: '0.9rem', px: 2, py: 2.5, borderRadius: '16px' }}
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 bg-slate-50 p-4 sm:p-6 rounded-3xl border border-slate-100">
            ...
            <SummaryCard label="Specjalizacja" value={getAppointmentSpecialization(appointment)} />
            <SummaryCard label="Typ wizyty" value={getAppointmentType(appointment)} />
            <SummaryCard label="Data" value={appointment.date} />
            <SummaryCard label="Godzina" value={`${appointment.startTime} - ${appointment.endTime}`} />
          </div>
...
const getAppointmentSpecialization = (appointment: BookAppointmentResultDto | AppointmentDetailDto) => {
