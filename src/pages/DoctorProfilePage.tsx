...
import {
  notificationsApi,
  type AppointmentNotificationDto,
...
const toDateString = (value: Dayjs | null) =>
  value ? value.format("YYYY-MM-DD") : "";
const toTimeString = (value: Dayjs | null) =>
...
const DoctorProfilePage = () => {
  ...
  const [selectedAppointment, setSelectedAppointment] =
    useState<DoctorAppointmentEvent | null>(null);
  const [appointmentAction, setAppointmentAction] =
    useState<AppointmentAction>("Confirmed");
  const [calendarDate, setCalendarDate] = useState(() =>
    dayjs().startOf("week").toDate(),
  );
  const [appointmentTypeModalOpen, setAppointmentTypeModalOpen] =
    useState(false);
  ...
  const createAppointmentTypeMutation = useMutation({
    ...
    onError: (error) => {
      setAppointmentTypeError(
        error instanceof Error
          ? error.message
          : "Nie udało się utworzyć typu wizyty.",
      );
    },
  });
  ...
  const updateAppointmentStatusMutation = useMutation({
    mutationFn: async (status: AppointmentAction) => {
      ...
      if (status === "Confirmed") {
        return notificationsApi.confirmAppointment(
          selectedAppointment.notificationId,
        );
      }

      return notificationsApi.cancelAppointment(
        selectedAppointment.notificationId,
      );
    },
    onSuccess: async () => {
  ...
  const handleSelectAppointment = (event: DoctorAppointmentEvent) => {
    ...
    setAppointmentAction(
      event.status === "Cancelled" ? "Confirmed" : "Confirmed",
    );
  };

  const editingLabel = scheduleDraft.scheduleId
    ? "Edytuj grafik"
    : "Dodaj grafik";
  ...
  const submitSchedule = () => {
    ...
    if (
      ...
      setScheduleError(
        'Pole "Obowiązuje do" nie może być wcześniejsze niż "Obowiązuje od".',
      );
      return;
    }
    ...
    saveScheduleMutation.mutate({
      ...
      validTo: scheduleDraft.validTo
        ? toDateString(scheduleDraft.validTo)
        : null,
      isActive: true,
    });
  };
  ...
  if (!isDoctor) {
    return (
      <Paper
        elevation={0}
        sx={{ p: 4, borderRadius: 3, border: "1px solid #dce5f2" }}
      >
        <Typography
          variant="h5"
          sx={{ fontWeight: 800, color: "#11223a", mb: 1 }}
        >
    ...
    >
      <Stack spacing={3}>
        <Box>
          ...
          <Typography sx={{ color: "#4f627a" }}>
            Zarządzaj opisem profilu i grafikami przypisanymi do konkretnych
            przychodni.
          </Typography>
        </Box>
          ...
          <Grid size={{ xs: 12, md: 4.2 }}>
            ...
            >
              <Stack spacing={1.5}>
                ...
                <FormControl
                  fullWidth
                  disabled={specializationsQuery.isLoading}
                >
                        ...
                        >
                          <Box>
                            ...
                            >
                              ...
                              )?.label ?? `Dzień ${schedule.dayOfWeek}`}
                            </Typography>
                            <Typography
                              sx={{
                                color: "#4f627a",
              ...
              <Stack spacing={2} sx={{ pt: 0.5 }}>
                ...
                >
                  ...
                  {!selectedAppointment.paymentStatus && (
                    <Typography
                      sx={{ color: "#888", fontStyle: "italic", fontSize: 14 }}
                    >
