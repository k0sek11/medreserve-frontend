...
const parseTime = (value: string) => {
  ...
  return dayjs().startOf("day").hour(Number.isFinite(hours) ? hours : 0).minute(Number.isFinite(minutes) ? minutes : 0);
};
...
const buildAppointmentEvent = (appointment: AppointmentNotificationDto): DoctorAppointmentEvent | null => {
...
const DoctorProfilePage = () => {
  ...
  const saveScheduleMutation = useMutation({
    ...
    onError: (error) => {
      setScheduleError(error instanceof Error ? error.message : "Nie udało się zapisać grafiku.");
    },
  });
  ...
  const appointmentEvents = useMemo(() => {
    return (appointmentNotificationsQuery.data ?? [])
      .filter((appointment) => !["Cancelled", "Canceled", "Rejected"].includes(appointment.status))
  ...
  const submitSchedule = () => {
    ...
    if (scheduleDraft.validTo && scheduleDraft.validTo.isBefore(scheduleDraft.validFrom, "day")) {
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Stack spacing={3}>
        ...
        <Paper elevation={0} sx={{ p: 2.5, borderRadius: 3, border: "1px solid #dce5f2" }}>
          <Stack spacing={2}>
            <Box>
              ...
              <Typography sx={{ color: "#4f627a" }}>
                Tygodniowy widok wszystkich wizyt, także oczekujących. Kliknij wizytę, żeby zmienić status.
              </Typography>
            </Box>
          ...
          <Grid size={{ xs: 12, md: 4.2 }}>
            <Paper elevation={0} sx={{ p: 2.5, borderRadius: 3, border: "1px solid #dce5f2" }}>
              <Stack spacing={1.5}>
                ...
                >
                  {saveProfileMutation.isPending ? "Zapisywanie..." : "Zapisz bio"}
                </Button>
            ...
            <Paper elevation={0} sx={{ p: 2.5, borderRadius: 3, border: "1px solid #dce5f2" }}>
              <Stack spacing={2}>
                <Box>
                  ...
                  <Typography sx={{ color: "#4f627a" }}>
                    Ustaw przychodnię, dzień tygodnia, zakres godzin i okres obowiązywania grafiku.
                  </Typography>
                </Box>
                ...
                {!clinicsQuery.isLoading && (clinicsQuery.data?.length ?? 0) === 0 ? (
                  <Alert severity="warning">
                    Najpierw zarejestruj lub dołącz do przychodni, aby móc dodać grafik.
                  </Alert>
                ) : null}
                ...
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <FormControl fullWidth required>
                      ...
                      <Select
                        ...
                        disabled={clinicsQuery.isLoading || (clinicsQuery.data?.length ?? 0) === 0}
                      >
                        {(clinicsQuery.data ?? []).map((clinic: ClinicListItemDto) => (
                          <MenuItem key={clinic.clinicId} value={String(clinic.clinicId)}>
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <FormControl fullWidth>
                      <InputLabel id="weekday-label">Dzień tygodnia</InputLabel>
                      <Select
                ...
                <Stack direction="row" spacing={1.2}>
                  <Button
                    ...
                    disabled={saveScheduleMutation.isPending || clinicsQuery.isLoading}
                    sx={{ textTransform: "none", fontWeight: 700 }}
                  >
                    {saveScheduleMutation.isPending ? "Zapisywanie..." : "Zapisz grafik"}
                  </Button>
                </Stack>
              </Stack>
            </Paper>

            <Paper elevation={0} sx={{ p: 2.5, borderRadius: 3, border: "1px solid #dce5f2", mt: 2.5 }}>
              <Stack spacing={1.5}>
                <Typography sx={{ fontWeight: 800, color: "#11223a", fontSize: 20 }}>
                  Aktualne grafiki
                </Typography>
                ...
                {schedulesQuery.isLoading ? (
                  <Typography sx={{ color: "#4f627a" }}>Ładowanie grafików...</Typography>
                ) : null}
                ...
                <Stack spacing={1.2}>
                  {orderedSchedules.map((schedule) => (
                    <Card key={schedule.scheduleId} variant="outlined" sx={{ borderRadius: 2 }}>
                      <CardContent>
                        <Stack
                        ...
                        >
                          <Box>
                            ...
                            <Typography sx={{ color: "#4f627a", fontWeight: 600 }}>
                              {schedule.clinicName ?? "Brak przypisanej przychodni"}
                            </Typography>
                            <Typography sx={{ color: "#4f627a" }}>
                              {schedule.startTime} - {schedule.endTime}
                            </Typography>
                            <Typography sx={{ color: "#4f627a", fontSize: 14 }}>
                              {schedule.validFrom} {schedule.validTo ? `- ${schedule.validTo}` : "- bez końca"}
                            </Typography>
                          </Box>
        ...
        <Dialog open={Boolean(selectedAppointment)} onClose={() => setSelectedAppointment(null)} fullWidth maxWidth="sm">
          <DialogTitle sx={{ fontWeight: 800, color: "#11223a" }}>Szczegóły wizyty</DialogTitle>
            ...
            {selectedAppointment ? (
              <Stack spacing={2} sx={{ pt: 0.5 }}>
                ...
                <Box>
                  ...
                  <Typography sx={{ fontWeight: 700 }}>{selectedAppointment.doctorName}</Typography>
                </Box>

                <Box>
                  <Typography sx={{ color: "#4f627a", fontSize: 14 }}>Status</Typography>
                  <Typography sx={{ fontWeight: 700 }}>{selectedAppointment.status}</Typography>
                </Box>
                ...
                <FormControl fullWidth>
                  <InputLabel id="appointment-action-label">Zmień status</InputLabel>
                  <Select
                    labelId="appointment-action-label"
                    label="Zmień status"
                    value={appointmentAction}
                    onChange={(event) => setAppointmentAction(event.target.value as AppointmentAction)}
                  >
          ...
          <DialogActions sx={{ px: 3, py: 2 }}>
            <Button onClick={() => setSelectedAppointment(null)} sx={{ textTransform: "none" }}>
              Zamknij
            </Button>
            <Button
              variant="contained"
              onClick={() => updateAppointmentStatusMutation.mutate(appointmentAction)}
              disabled={updateAppointmentStatusMutation.isPending || !selectedAppointment}
              sx={{ textTransform: "none", fontWeight: 700 }}
            >
