...
const DoctorDetailsPage = () => {
  ...
  const activeAppointmentTypeId =
    selectedAppointmentTypeId ?? profileQuery.data?.appointmentTypes[0]?.appointmentTypeId ?? null;
  ...
  const selectedClinic = useMemo(() => {
    return profileQuery.data?.clinics.find((clinic) => clinic.clinicId === selectedClinicId) ?? null;
  }, [profileQuery.data?.clinics, selectedClinicId]);
  ...
  const bookMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ["doctor-availability", parsedDoctorId] });
      await queryClient.invalidateQueries({ queryKey: ["doctor-availability-calendar", parsedDoctorId] });
    ...
    onError: (error) => {
      setBookingMessage(error instanceof Error ? error.message : "Nie udało się zarezerwować wizyty.");
    },
  });
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Card elevation={0} sx={{ border: "1px solid #dce5f2", borderRadius: 3 }}>
              ...
              <CardContent>
                <Stack spacing={1.2}>
                  <Box>
                    <Typography sx={{ fontSize: 30, fontWeight: 800, color: "#11223a", lineHeight: 1.15 }}>
                      {profileQuery.data.fullName}
                    </Typography>
                    <Typography sx={{ color: "#5a6e86", fontSize: 18 }}>
                      {profileQuery.data.specializations.join(", ") || "Brak specjalizacji"}
                    </Typography>
                  </Box>
                  ...
                  <Stack spacing={0.8}>
                    <RoundIconInfo icon="P" text={profileQuery.data.phoneNumber ?? "Brak telefonu"} />
                    <RoundIconInfo icon="M" text={profileQuery.data.city ?? "Brak miasta"} />
                    <RoundIconInfo
                      icon="A"
                      text={profileQuery.data.streetAddress ?? "Brak adresu placówki"}
                    />
                  </Stack>
              ...
              <Paper elevation={0} sx={{ border: "1px solid #dce5f2", borderRadius: 3, p: 2.5 }}>
                <Stack spacing={1.5}>
                  ...
                  {profileQuery.data.clinics.length === 0 ? (
                    <Alert severity="info">Ten lekarz nie ma jeszcze przypisanej przychodni.</Alert>
                    ...
                    <Grid container spacing={2}>
                      <Grid size={{ xs: 12, lg: 4 }}>
                        <Stack spacing={2}>
                          ...
                          {selectedClinic ? (
                            <Typography sx={{ color: "#5a6e86", fontSize: 13 }}>
                          ...
                          <Select
                            ...
                            onChange={(event) => {
                              setSelectedAppointmentTypeId(Number(event.target.value));
                              setSelectedSlotId(null);
                              setBookingMessage(null);
                            }}
                            fullWidth
                          ...
                          >
                            ...
                            {profileQuery.data.appointmentTypes.map((type) => (
                              <MenuItem key={type.appointmentTypeId} value={type.appointmentTypeId}>
                                {type.name} • {formatMoney(type.basePrice)} • {type.durationMinutes} min
                              </MenuItem>
                            ))}
                          </Select>
                      ...
                      <Grid size={{ xs: 12, lg: 8 }}>
                        <Stack spacing={1.5}>
                          <Box>
                            ...
                            <Typography sx={{ color: "#11223a", fontWeight: 700, fontSize: 20 }}>
                              {selectedDate.format("DD.MM.YYYY")}
                            </Typography>
                          </Box>

                          {selectedClinicId === null ? (
                            <Alert severity="info">Wybierz przychodnię, aby zobaczyć dostępne terminy.</Alert>
                          ...
                          {selectedClinicId !== null && !availabilityQuery.isLoading && availabilityQuery.data ? (
                            <>
                              <Stack direction="row" spacing={1} useFlexGap sx={{ flexWrap: "wrap" }}>
                                <Chip label="Wolny" sx={{ bgcolor: "#e8f7ee", color: "#1f9b45" }} />
                                <Chip label="Zajęty" sx={{ bgcolor: "#edf2f7", color: "#73859c" }} />
                              </Stack>

                              {availableSlots.length === 0 ? (
                                <Alert severity="info">
                                  Brak wolnych terminów dla wybranej daty i typu wizyty.
                                </Alert>
                              ) : (
                                <Grid container spacing={1}>
                                  {availabilityQuery.data.slots.map((slot) => (
                                    <Grid key={slot.timeSlotId} size={{ xs: 6, sm: 4, md: 3 }}>
                                      <Button
                                        fullWidth
                                        disabled={slot.isBooked}
                                        variant={selectedSlotId === slot.timeSlotId ? "contained" : "outlined"}
                                        onClick={() => {
                                          if (!slot.isBooked) {
                                        ...
                                        sx={{
                                          ...
                                          borderColor: slot.isBooked ? "#d7e0eb" : undefined,
                          ...
                          {bookingMessage ? <Alert severity="success">{bookingMessage}</Alert> : null}
                          {bookMutation.isError ? (
                            <Alert severity="error">Nie udało się zarezerwować wizyty.</Alert>
                          ) : null}
                          ...
                          >
                            {bookMutation.isPending ? "Rezerwowanie..." : "Zarezerwuj wizytę"}
                          </Button>
                        </Stack>
                      </Grid>
                    </Grid>
...
const BookingCalendarDay = ({ availableDays, day, outsideCurrentMonth, ...other }: BookingCalendarDayProps) => {
