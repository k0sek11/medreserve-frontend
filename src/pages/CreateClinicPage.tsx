...
export const CreateClinicPage = () => {
  ...
  const createClinicMutation = useMutation({
    ...
    onError: (error) => {
      setSubmitError(error instanceof Error ? error.message : "Nie udało się utworzyć przychodni.");
    },
  });
  ...
  const openingHoursInvalid = Boolean(openingFrom && openingTo && openingTo.isBefore(openingFrom));
  const canSubmit =
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: 760, mx: "auto", p: 2 }}>
        <Paper elevation={0} sx={{ p: { xs: 3, md: 4 }, border: "1px solid #dce5f2", borderRadius: 3 }}>
          <Stack spacing={3}>
            <Box>
              <Typography variant="h4" component="h1" sx={{ fontWeight: 800, color: "#11223a" }}>
                Zarejestruj przychodnię
              </Typography>
              <Typography sx={{ color: "#4f627a", mt: 1 }}>
                Uzupełnij dane podstawowe. Klinika zostanie utworzona jako aktywna i przypisana do Twojego profilu lekarza.
              </Typography>
            </Box>
            ...
            <Stack spacing={2}>
                ...
                >
                  {citiesQuery.isLoading && (
                    <MenuItem disabled value="">
                      <CircularProgress size={20} sx={{ mr: 2 }} /> Ładowanie miast...
                    </MenuItem>
                  )}
              ...
              <TextField
                ...
                error={phoneError}
                helperText={phoneError ? "Podaj poprawny numer telefonu lub zostaw pole puste." : ""}
                fullWidth
              />
            ...
            <Stack direction="row" spacing={2} sx={{ justifyContent: "flex-end" }}>
              ...
              >
                {createClinicMutation.isPending ? "Zapisywanie..." : "Zapisz przychodnię"}
              </Button>
            </Stack>
          </Stack>
        </Paper>
